...
Renaming `version` parameter in `Application.__init__(version: int = pyteal.MAX_VERSION)` to `teal_version`, to be more explicit. Otherwise developers may be confused that it's the version of the specific smart contract. It may be desirable to allow `version` to continue to be specified for some time, but to raise a `DeprecationWarning`. 
